package com.paultondeur.away3d.robotlegs.base {
	...
	public class Mediator3DMap implements IMediator3DMap {
		...
		public function get scene3D() : away3d.containers.Scene3D {
			return _scene3D;
		}
		...
		public function Mediator3DMap(scene3D : Scene3D, injector : IInjector, reflector : IReflector) {
			_injector = injector;
		...
		protected function removeSceneListeners() : void {
			if (scene3D) {
				scene3D.removeEventListener(Scene3DEvent.ADDED_TO_SCENE, onViewAdded);
				scene3D.removeEventListener(Scene3DEvent.REMOVED_FROM_SCENE, onViewRemoved);
			}
		}

		protected function addSceneListeners() : void {
			if (scene3D) {
				scene3D.addEventListener(Scene3DEvent.ADDED_TO_SCENE, onViewAdded);
				scene3D.addEventListener(Scene3DEvent.REMOVED_FROM_SCENE, onViewRemoved);
			}
		}
